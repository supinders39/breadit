        ...
        <div className='flex flex-col gap-y-6 mt-4'>
            {comments.filter((comment) => !comment.replyToId).map((topLevelComment) => {
                ...
                return <div key={topLevelComment.id} className='flex flex-col '>
                    <div className='mb-2'>
                        <PostComment currentVote={topLevelCommentVote} votesAmt={topLevelCommentVotesAmt} postId={postId} comment={topLevelComment} />
                    </div>
                    ...
                    {topLevelComment.replies.sort((a, b) => b.votes.length - a.votes.length).map((reply) => {
                        ...
                        return <div key={reply.id} className='ml-2 py-2 pl-4 border-l-2 border-zinc-200'>
                            <PostComment currentVote={replyVote} votesAmt={replyVotesAmt} postId={postId} comment={reply} />
                        </div>
                    })}
                </div>
            })}
